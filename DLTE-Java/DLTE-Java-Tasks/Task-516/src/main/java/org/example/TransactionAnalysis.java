...
public class TransactionAnalysis implements Runnable {
    ...
    @Override
    public void run() {//cli where all the tasks to be executed is listed...entry point in threading concept
        lock.lock();
    ...
    //these methods are additional to define method reference approach
    public void displayAmount() {
        int index=0;
        for(Transaction each:analysis){
            System.out.println("Transaction "+(index+1)+ " is "+each.getTransactionAmount());
            index++;
        }
    }

    public void displayReciepientNames() {
        int index=0;
        for(Transaction each:analysis){
            System.out.println("Reciepient "+(index+1)+" is "+each.getReciept());
            index++;
        }
    }

    public void displatTransactionDates() {
        int index=0;
        for(Transaction each:analysis){
            System.out.println("Transaction date "+(index+1)+" is "+each.getDateOfTransaction());
            index++;
        }
    }
}
